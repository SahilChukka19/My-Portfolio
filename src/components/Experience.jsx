...
const Experience = () => {
    ...
    const carX = useTransform(scrollYProgress, [0, 1], ["2%", "98%"]);

    return (
        <section id="experience" ref={targetRef} className="relative bg-slate-950">
            {/* Desktop Horizontal View */}
            <div className="hidden lg:block h-[350vh]">
                <div className="sticky top-0 h-screen flex items-center overflow-hidden">
                    ...
                    <div className="relative w-full h-full">
                        ...
                        >
                            <img src="/icons/f1_car.svg" alt="F1 Car" className="w-full h-auto object-contain" />
                            <motion.div
            ...
            <div className="lg:hidden py-24 px-6 relative">
                <div className="max-w-7xl mx-auto">
                    ...
                    >
                        ...
                        <div className="w-20 h-1 bg-cyan-500 mt-2" />
                    </motion.div>

                    <div className="relative space-y-12 pl-6 border-l-2 border-slate-800">
                        {EXPERIENCES.map((exp, idx) => (
                            <motion.div
                                ...
                                className="relative"
                            >
                                <div className="absolute -left-[33px] top-6 w-4 h-4 rounded-full bg-cyan-500 shadow-[0_0_15px_#06b6d4] z-10" />
                                <div className="bg-slate-900 border border-white/10 p-6 rounded-2xl relative">
                                    <div className="flex flex-col gap-2 mb-4">
                                        <div className="text-xs font-mono text-cyan-400">{exp.date}</div>
                                        <h3 className="text-xl font-bold text-white">{exp.role}</h3>
                                        <div className="flex items-center gap-2 text-slate-400 text-sm">
                                            <MapPin className="w-3 h-3 text-cyan-500" /> {exp.company}
                                        </div>
                                    </div>
                                    <p className="text-slate-400 text-sm leading-relaxed mb-4">{exp.desc}</p>
                                    <div className="flex flex-wrap gap-2">
                                        {exp.tech.map(t => (
                                            <span key={t} className="px-2 py-0.5 bg-slate-950 border border-slate-800 rounded text-cyan-500/80 text-[10px] font-mono">
                                                {t}
                                            </span>
                                        ))}
                                    </div>
